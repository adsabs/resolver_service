...
class TestResolverNew(TestCaseDatabase):
    ...
    # the following four types' links are created on the fly
    # unlike the first version, everything has to come from database and
    # since 1987gady.book.....B is not in database should return 404
    def test_fetchingAbstract(self):
        response = LinkRequest('1987gady.book.....B', 'ABSTRACT').process_request_new()
        self.assertEqual(response._status_code, 404)
        self.assertDictEqual(eval(response.response[0]), {'error': 'did not find any records'})
    def test_fetchingCitations(self):
        response = LinkRequest('1987gady.book.....B', 'CITATIONS').process_request_new()
        self.assertEqual(response._status_code, 404)
        self.assertDictEqual(eval(response.response[0]),{'error': 'did not find any records'})
    def test_fetchingCoRead(self):
        response = LinkRequest('1987gady.book.....B', 'COREADS').process_request_new()
        self.assertEqual(response._status_code, 404)
        self.assertDictEqual(eval(response.response[0]), {'error': 'did not find any records'})
    def test_fetchingReferences(self):
        response = LinkRequest('1998ARA&A..36..189K', 'REFERENCES').process_request_new()
        self.assertEqual(response._status_code, 404)
        self.assertDictEqual(eval(response.response[0]), {'error': 'did not find any records'})
    ...
    def test_publicMethodLinkRequest1(self):
        ...
        self.assertEqual(response.response[0], b'{"error": "did not find any records"}')
    def test_publicMethodLinkRequest2(self):
        response = LinkRequest('2003MNRAS.342.1117M', 'CD').verify_url_new('http%3A%2F%2Fvizier.u-strasbg.fr%2Fviz-bin%2FVizieR%3F-source%3DIV%2F26')
        self.assertEqual(response._status_code, 200)
        self.assertEqual(response.response[0], b'{"link": "verified"}')

...
class PopulateRequest():
    ...
    def process_request(self, records):
        ...
        if len(records) > current_app.config['RESOLVER_MAX_RECORDS_ADD']:
            return self.__return_response({'error': 'too many records to add to db at one time, recived %s reocrds while the limit is %s'%(len(records), current_app.config['RESOLVER_MAX_RECORDS_ADD'])}, 400)
